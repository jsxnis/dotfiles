...
def python_fu_arrow_from_selection(img, layer, arrowangle, arrowsize,
    ...
    #
    strokes = [ x1, y1, x2, y2 ]
    # If the line is vertical, don't try to calculate slope:
    dy = y2 - y1
    dx = x2 - x1
    if dx == 0 :     # vertical
        xslop = 0
        ...
        yslop = 0
    elif dy > dx :   # mainly vertical
        yslop = int(arrowsize / 2)
        xslop = yslop * abs(float(dx) / float(dy))
    else :           # dx > dy, mainly horizontal
        xslop = int(arrowsize / 2)
        yslop = xslop * abs(float(dy) / float(dx))
